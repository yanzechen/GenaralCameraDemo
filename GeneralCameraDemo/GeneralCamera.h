...
*	Parameters:	每帧预览图像数据，数据长度，自定义参数(在CameraInit函数中指定)
*	Return :			被动调用
*/
typedef int (CALLBACK* DS_SNAP_PROC)(BYTE *pImageBuffer, LONG BufferLength, VOID* pVoid);
...
*	Parameters:	相机编号，回调函数，回调函数自定义参数，预览窗口句柄（无需预览则NULL）
*/
...
/*
*	Description:		写IIC
*	Parameters:	相机编号，地址，地址长度，值，值长度，器件地址
*/
EXTERN_C CAMERA_STATUS CameraWriteIIC(BYTE CamNum,
	LONG lAddress, LONG lAddrNum, IN LONG* lData, LONG lDataNum, BYTE slaveID);

/*
*	Description:		读IIC
*	Parameters:	相机编号，地址，地址长度，值，值长度，器件地址
*/
EXTERN_C CAMERA_STATUS CameraReadIIC(BYTE CamNum,
	LONG lAddress, LONG lAddrNum, OUT LONG* lData, LONG lDataNum, BYTE slaveID);